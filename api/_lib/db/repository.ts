...
export interface CompetitorRepo {
  /** All competitors, in insertion-independent order. */
  ...
  /** Merge `patch` onto an existing record (id is immutable); `null` if absent. */
  update(id: string, patch: Partial<Competitor>): Promise<Competitor | null>;
...
export function drizzleRepo(db: Db): CompetitorRepo {
  const repo: CompetitorRepo = {
    ...
    async update(id, patch) {
      ...
      // Re-validate the merged record; `id` stays fixed so identity is immutable.
      const merged = parseCompetitor({ ...existing, ...patch, id });
      return repo.upsert(merged);
    },
