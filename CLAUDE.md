...
- [Feature 015 — Read the user guide in-app](docs/requirements/feature-015-user-guide-in-app.md)
...
- [015-user-guide-in-app](docs/retrospectives/015-user-guide-in-app.md) — a **User guide** nav button opens `docs/user-guide.md` in a `<dialog>` modal. Dependency-free (no ADR): pure node-tested `domain/markdown.ts` (`parseMarkdown`→block AST + recursive `parseInline` spans; only the guide's subset) renders the raw `?raw`-imported doc as **real React elements** (no `dangerouslySetInnerHTML`), single source of truth. `?raw` from repo-root `docs/` (outside the Vite root) works via default `fs.allow` + `vite/client` types. Recursive `strong`/`em` for the guide's `**bold *em***`. 226→238 tests. Lesson (cf. 014): preview screenshots wedge — verified the modal structurally via `javascript_tool`.
